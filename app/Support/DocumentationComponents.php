...
BLADE,
            ...
            'modal' => [
                ...
                'description' => 'Use para formularios curtos, confirmacoes e fluxos que precisam interromper a pagina sem sair do contexto. O painel usa camada dinamica a partir de 200, permitindo menus do modal ativo e modais aninhados sem z-index maximo.',
...
BLADE,
            ...
            'sidebar' => [
                ...
                'summary' => 'Navegacao lateral responsiva de 18rem, sem faixa externa, com estados hover/ativo circulares e saida outline danger.',
                'description' => 'Use em areas autenticadas e dashboards. A Sidebar nao cria fundo fora da superficie principal; o item ativo reutiliza o circulo do hover, o gap entre icone e texto e 30% menor, e a saida usa outline danger sem sombra.',
...
BLADE,
            ...
            'drawer' => [
                ...
                'description' => 'Use para filtros, formularios auxiliares, detalhes de registro e fluxos secundarios sem tirar o usuario da tela atual. O painel usa camada dinamica e preserva o scroll interno sem cortar selects teletransportados.',
    ...
    private function withGuidance(array $components): array
    {
        $guidance = [
            ...
            'sidebar' => [
                ...
                'O estado ativo deve permanecer restrito ao circulo do icone, igual ao hover, sem fundo no link inteiro.',
                'O link de saida ja usa outline danger com `shadow-none`.',
...
BLADE,
    ...
    private function synchronizeRegistryContract(array $documentation, array $registry): array
    {
        ...
        $documentation['props'] = collect($registry['props'] ?? [])
            ->map(function (string $prop) use ($documentedProps, $slug): array {
                $documented = $documentedProps->get($this->normalizedPropName($prop));

                if ($documented && $documented['notes'] !== 'Prop pública registrada no pacote SampaUI.' && $documented['default'] !== '-') {
                    $documented['name'] = $prop;

                    return $documented;
                }

                $meta = $this->getPropMetadata($slug, $prop);

                return [
                    'name' => $prop,
                    'type' => $documented['type'] ?? $meta['type'],
                    'default' => ($documented['default'] ?? '-') !== '-' ? $documented['default'] : $meta['default'],
                    'notes' => ($documented['notes'] ?? '') !== '' && ($documented['notes'] ?? '') !== 'Prop pública registrada no pacote SampaUI.'
                        ? $documented['notes']
                        : $meta['notes'],
                ];
            })
    ...
    private function componentFromRegistry(string $slug, array $component): array
    {
        ...
        return [
            ...
            'props' => collect($component['props'] ?? [])
                ->map(fn (string $prop): array => array_merge(['name' => $prop], $this->getPropMetadata($slug, $prop)))
                ->values()
                ->all(),
            'attributes' => ['class', 'id', 'wire:*', 'x-*', 'aria-*', 'data-*'],
            'accessibility' => [
    ...
    private function getPropMetadata(string $slug, string $prop): array
    {
        $map = [
            'name' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Nome do campo no formulário e chave vinculada ao ErrorBag.'],
            'label' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Rótulo textual exibido acima ou ao lado do componente.'],
            'value' => ['type' => 'mixed', 'default' => 'null', 'notes' => 'Valor inicial quando não estiver utilizando wire:model.'],
            'placeholder' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Texto de orientação exibido quando o campo está vazio.'],
            'hint' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Texto informativo auxiliar posicionado abaixo do componente.'],
            'error' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Mensagem customizada de erro de validação.'],
            'state' => ['type' => 'valid|invalid|null', 'default' => 'null', 'notes' => 'Estado visual explícito de validação do campo.'],
            'disabled' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Desabilita a interação e aplica estilo desativado.'],
            'readonly' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Bloqueia alteração mantendo o controle focalizável.'],
            'required' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Adiciona indicador de obrigatoriedade e validação nativa.'],
            'loading' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe spinner ou estado de carregamento durante requisições.'],
            'loading-target' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Ação do Livewire monitorada para feedback de loading (wire:target).'],
            'clearable' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe botão rápido para limpar o valor selecionado ou digitado.'],
            'clear-label' => ['type' => 'string', 'default' => 'Limpar', 'notes' => 'Rótulo acessível do botão de limpar.'],
            'icon' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Nome do ícone Bootstrap Icons (sem o prefixo bi-).'],
            'size' => ['type' => 'xs|sm|md|lg|xl', 'default' => 'md', 'notes' => 'Dimensão física e espaçamento interno do componente.'],
            'variant' => ['type' => 'string', 'default' => 'primary', 'notes' => 'Variante de cor e tema semântico do componente.'],
            'color' => ['type' => 'string', 'default' => 'primary', 'notes' => 'Cor de destaque aplicada quando ativo ou selecionado.'],
            'model' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Nome da propriedade booleana Livewire que controla a visibilidade.'],
            'title' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Título principal exibido no cabeçalho.'],
            'subtitle' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Subtítulo descritivo complementar.'],
            'description' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Descrição explicativa do conteúdo.'],
            'items' => ['type' => 'array', 'default' => '[]', 'notes' => 'Lista de itens ou opções estruturadas em array.'],
            'options' => ['type' => 'array', 'default' => '[]', 'notes' => 'Array associativo de opções [ valor => label ] ou coleções.'],
            'rows' => ['type' => 'int|array', 'default' => '3', 'notes' => 'Linhas da tabela ou quantidade de linhas do textarea.'],
            'columns' => ['type' => 'array', 'default' => '[]', 'notes' => 'Definição de colunas com labels, keys e alinhamentos.'],
            'auto-resize' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Ajusta automaticamente a altura da textarea ao digitar.'],
            'counter' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe contador de caracteres digitados no rodapé.'],
            'maxlength' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Limite máximo de caracteres permitidos.'],
            'copyable' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe botão para copiar o conteúdo para a área de transferência.'],
            'copy-label' => ['type' => 'string', 'default' => 'Copiar', 'notes' => 'Rótulo acessível do botão de cópia.'],
            'resize' => ['type' => 'none|vertical|horizontal|both', 'default' => 'vertical', 'notes' => 'Direção permitida de redimensionamento manual pelo usuário.'],
            'min-rows' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Limite mínimo de linhas no auto-resize.'],
            'max-rows' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Limite máximo de linhas no auto-resize antes de ativar rolagem.'],
            'revealable' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe botão de olho para alternar a visibilidade de senhas.'],
            'persistent' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Impede o fechamento ao clicar no backdrop ou apertar Escape.'],
            'close-on-escape' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Fecha o overlay ao pressionar a tecla Escape.'],
            'close-on-outside' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Fecha o overlay ao clicar fora da área principal.'],
            'close-button' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Exibe botão de fechar (X) no cabeçalho.'],
            'close-event' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Evento JavaScript emitido no navegador ao fechar.'],
            'after-close' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Expressão Alpine ou callback executada após a animação de saída.'],
            'panel-class' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Classes Tailwind adicionais aplicadas no container interno.'],
            'backdrop-class' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Classes Tailwind adicionais para o fundo escurecido.'],
            'placement' => ['type' => 'string', 'default' => 'right', 'notes' => 'Posição de ancoragem ou direção de abertura do painel/menu.'],
            'symbol' => ['type' => 'string', 'default' => 'R$', 'notes' => 'Símbolo monetário exibido no prefixo do campo.'],
            'searchable' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Ativa campo de busca em tempo real na toolbar.'],
            'search-model' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Binding reativo Livewire com debounce automático.'],
            'search-placeholder' => ['type' => 'string', 'default' => 'Buscar...', 'notes' => 'Texto de orientação do campo de pesquisa.'],
            'search-name' => ['type' => 'string', 'default' => 'search', 'notes' => 'Nome do input de busca.'],
            'search' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Valor atual do termo pesquisado.'],
            'selectable' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Ativa seleção múltipla de linhas com checkboxes.'],
            'selected-rows' => ['type' => 'array', 'default' => '[]', 'notes' => 'Array com chaves dos registros selecionados.'],
            'select-name' => ['type' => 'string', 'default' => 'selected', 'notes' => 'Nome do campo de seleção de linhas.'],
            'row-key' => ['type' => 'string', 'default' => 'id', 'notes' => 'Chave única usada como identificador de cada linha.'],
            'export-href' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL para exportação de dados (ex: CSV ou Excel).'],
            'export-label' => ['type' => 'string', 'default' => 'Exportar', 'notes' => 'Rótulo textual do botão de exportação.'],
            'sticky-header' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Fixa o cabeçalho no topo durante a rolagem.'],
            'mobile-cards' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Converte as linhas da tabela em cards elegantes no mobile.'],
            'compact' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Reduz o padding e tipografia das células.'],
            'bordered' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Aplica bordas externas e divisores nas células.'],
            'flush' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Remove bordas externas para embutir perfeitamente em Cards.'],
            'bleed' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Estende a tabela até as margens do container.'],
            'striped' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Alterna fundo discreto em linhas pares.'],
            'hover' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Aplica realce ao passar o mouse sobre as linhas.'],
            'sort-by' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Nome da coluna ordenada atualmente.'],
            'sort-direction' => ['type' => 'asc|desc', 'default' => 'asc', 'notes' => 'Direção ativa da ordenação.'],
            'sort-method' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Método Livewire chamado ao ordenar colunas.'],
            'per-page' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Quantidade de registros exibidos por página.'],
            'page' => ['type' => 'int', 'default' => '1', 'notes' => 'Número da página atual.'],
            'total' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Total geral de registros para cálculo de páginas.'],
            'pagination-method' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Método Livewire chamado ao navegar entre páginas.'],
            'pagination-type' => ['type' => 'numbered|simple', 'default' => 'numbered', 'notes' => 'Estilo dos controles de paginação.'],
            'empty' => ['type' => 'string', 'default' => 'Nenhum registro encontrado.', 'notes' => 'Mensagem simples quando a lista está vazia.'],
            'empty-title' => ['type' => 'string', 'default' => 'Nenhum registro encontrado', 'notes' => 'Título do estado vazio.'],
            'empty-description' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Descrição do estado vazio.'],
            'empty-icon' => ['type' => 'string', 'default' => 'inbox', 'notes' => 'Ícone Bootstrap Icons do estado vazio.'],
            'pulse' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Aplica animação suave de pulsação luminosa.'],
            'max' => ['type' => 'int|float', 'default' => '100', 'notes' => 'Valor máximo correspondente a 100% da barra.'],
            'show-value' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe o valor percentual numérico.'],
            'lines' => ['type' => 'int', 'default' => '1', 'notes' => 'Quantidade de linhas de placeholder animadas.'],
            'circle' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Formato circular para simular avatares e ícones.'],
            'steps' => ['type' => 'array', 'default' => '[]', 'notes' => 'Etapas do progresso com labels e descrições.'],
            'current' => ['type' => 'int', 'default' => '1', 'notes' => 'Índice da etapa ativa atual (iniciando em 1).'],
            'tabs' => ['type' => 'array', 'default' => '[]', 'notes' => 'Abas disponíveis [ chave => Rótulo ].'],
            'active' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Identificador da aba inicialmente ativa.'],
            'text' => ['type' => 'string', 'default' => '', 'notes' => 'Texto explicativo exibido no tooltip.'],
            'position' => ['type' => 'string', 'default' => 'top', 'notes' => 'Posição do elemento (top, bottom, left, right).'],
            'duration' => ['type' => 'int', 'default' => '4000', 'notes' => 'Duração em milissegundos antes de ocultar automaticamente.'],
            'align' => ['type' => 'left|right|center', 'default' => 'left', 'notes' => 'Alinhamento horizontal em relação ao botão trigger.'],
            'width' => ['type' => 'string', 'default' => 'auto', 'notes' => 'Largura do menu flutuante.'],
            'danger' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Aplica estilo visual de ação destrutiva (vermelho).'],
            'href' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL de destino para links de navegação.'],
            'logo' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL de arquivo SVG ou imagem de logotipo.'],
            'alt' => ['type' => 'string', 'default' => 'SampaUI', 'notes' => 'Texto alternativo para acessibilidade.'],
            'brand-href' => ['type' => 'string', 'default' => '/', 'notes' => 'URL de destino ao clicar na marca/logotipo.'],
            'logo-src' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL da imagem do logotipo.'],
            'logo-alt' => ['type' => 'string', 'default' => 'SampaUI', 'notes' => 'Texto alternativo do logotipo.'],
            'sections' => ['type' => 'array', 'default' => '[]', 'notes' => 'Seções agrupadas de itens de navegação.'],
            'user' => ['type' => 'array|null', 'default' => 'null', 'notes' => 'Dados do usuário autenticado para o rodapé.'],
            'avatar' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL da foto do avatar.'],
            'avatar-alt' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Texto alternativo da foto do avatar.'],
            'initial-state' => ['type' => 'expanded|collapsed', 'default' => 'expanded', 'notes' => 'Estado de expansão inicial da barra lateral.'],
            'collapsed' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Inicia a barra em modo recolhido (ícones).'],
            'collapsible' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Permite expandir/recolher a barra com botão dedicado.'],
            'open-event' => ['type' => 'string', 'default' => 'sampaui:open', 'notes' => 'Evento emitido ao expandir ou abrir.'],
            'state-event' => ['type' => 'string', 'default' => 'sampaui:state', 'notes' => 'Evento emitido com o estado atual.'],
            'active-color' => ['type' => 'string', 'default' => 'primary', 'notes' => 'Cor de destaque do item ativo.'],
            'logout-href' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL de encerramento de sessão.'],
            'logout-label' => ['type' => 'string', 'default' => 'Sair', 'notes' => 'Rótulo acessível do botão de logout.'],
            'status' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Indicador de presença ou status de contexto.'],
            'eyebrow' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Texto pequeno acima do título principal.'],
            'menu' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe botão hamburguer de menu mobile.'],
            'menu-event' => ['type' => 'string', 'default' => 'sampaui:sidebar-toggle', 'notes' => 'Evento emitido ao clicar no menu mobile.'],
            'aria-label' => ['type' => 'string', 'default' => 'Container', 'notes' => 'Rótulo de acessibilidade para leitores de tela.'],
            'notifications' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe botão de notificações no cabeçalho.'],
            'notification-count' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Contador numérico de notificações não lidas.'],
            'notification-event' => ['type' => 'string', 'default' => 'sampaui:notifications-open', 'notes' => 'Evento emitido ao clicar no sino de notificações.'],
            'sticky' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Fixa o elemento no topo durante a rolagem.'],
            'id' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Identificador único no DOM.'],
            'label-for' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'ID do elemento associado para acessibilidade.'],
            'length' => ['type' => 'int', 'default' => '6', 'notes' => 'Quantidade de dígitos ou blocos de entrada.'],
            'prefix' => ['type' => 'string', 'default' => 'pin', 'notes' => 'Prefixo para identificadores únicos de cada dígito.'],
            'clear' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Habilita atalho para limpar o código preenchido.'],
            'numbers' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Permite exclusivamente caracteres numéricos (0-9).'],
            'letters' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Permite caracteres alfabéticos (A-Z).'],
            'smart' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Avanço automático de foco e colagem inteligente do clipboard.'],
            'inline' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Organiza as opções horizontalmente lado a lado.'],
            'paginator' => ['type' => '\Illuminate\Contracts\Pagination\Paginator|null', 'default' => 'null', 'notes' => 'Instância de paginação nativa do Eloquent/Laravel.'],
            'current-page' => ['type' => 'int', 'default' => '1', 'notes' => 'Número da página atual.'],
            'last-page' => ['type' => 'int', 'default' => '1', 'notes' => 'Número da última página disponível.'],
            'previous-url' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL para a página anterior.'],
            'next-url' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'URL para a próxima página.'],
            'window' => ['type' => 'int', 'default' => '2', 'notes' => 'Quantidade de páginas vizinhas visíveis na paginação.'],
            'wire-method' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Método Livewire chamado ao navegar entre páginas.'],
            'simple' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Modo compacto contendo apenas botões Anterior e Próxima.'],
            'appearance' => ['type' => 'string', 'default' => 'subtle', 'notes' => 'Estilo visual de preenchimento e superfície.'],
            'divided' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Aplica divisores visuais entre seções.'],
            'overflow' => ['type' => 'visible|hidden|auto', 'default' => 'visible', 'notes' => 'Comportamento de overflow do container.'],
            'padding' => ['type' => 'none|sm|md|lg|xl', 'default' => 'md', 'notes' => 'Espaçamento interno do container.'],
            'from' => ['type' => 'contact|me|system', 'default' => 'contact', 'notes' => 'Origem da mensagem (contato, própria ou sistema).'],
            'author' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Nome do autor exibido acima da bolha.'],
            'time' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Horário de envio da mensagem (ex: 10:45).'],
            'show-avatar' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe avatar circular ao lado da mensagem.'],
            'button-label' => ['type' => 'string', 'default' => 'Enviar', 'notes' => 'Rótulo acessível do botão de envio.'],
            'button-icon' => ['type' => 'string', 'default' => 'send-fill', 'notes' => 'Ícone do botão de envio.'],
            'submit-on-enter' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Envia o formulário ao pressionar Enter.'],
            'show-counter' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Exibe contador de caracteres digitados.'],
            'max-length' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Quantidade máxima de caracteres permitidos.'],
            'back-button' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Exibe botão de voltar para a lista no mobile.'],
            'auto-scroll' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Executa rolagem automática até a última mensagem.'],
            'conversations' => ['type' => 'array', 'default' => '[]', 'notes' => 'Array de conversas para a sidebar.'],
            'height' => ['type' => 'string', 'default' => '100%', 'notes' => 'Altura total do container.'],
            'mobile-panel' => ['type' => 'sidebar|conversation', 'default' => 'conversation', 'notes' => 'Painel visível por padrão em telas mobile.'],
            'multiple' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Permite a seleção ou abertura de múltiplos itens simultaneamente.'],
            'accept' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Tipos MIME ou extensões de arquivo aceitos.'],
            'preview' => ['type' => 'bool', 'default' => 'true', 'notes' => 'Exibe pré-visualização de arquivos ou imagens selecionadas.'],
            'max-size' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Tamanho máximo permitido em Megabytes (MB).'],
            'retry' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Habilita botão de tentar novamente em falhas de upload.'],
            'chunk-size' => ['type' => 'int|null', 'default' => 'null', 'notes' => 'Tamanho do bloco em bytes para uploads fragmentados.'],
            'type-error' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Mensagem para formatos de arquivos inválidos.'],
            'size-error' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Mensagem para arquivos que excedem o tamanho máximo.'],
            'cancel-label' => ['type' => 'string', 'default' => 'Cancelar', 'notes' => 'Rótulo do botão de cancelar upload.'],
            'retry-label' => ['type' => 'string', 'default' => 'Tentar novamente', 'notes' => 'Rótulo do botão de reenvio.'],
            'remove-name' => ['type' => 'string', 'default' => 'remove_avatar', 'notes' => 'Nome do campo enviado quando o avatar é removido.'],
            'remove-model' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Propriedade Livewire vinculada à remoção do avatar.'],
            'remove-label' => ['type' => 'string', 'default' => 'Remover', 'notes' => 'Rótulo do botão de remoção de foto.'],
            'upload-label' => ['type' => 'string', 'default' => 'Alterar foto', 'notes' => 'Rótulo do botão de seleção de foto.'],
            'help' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Instrução auxiliar descritiva.'],
            'min' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Valor mínimo aceitável pelo campo.'],
            'max' => ['type' => 'string|null', 'default' => 'null', 'notes' => 'Valor máximo aceitável pelo campo.'],
            'search-placeholder' => ['type' => 'string', 'default' => 'Buscar...', 'notes' => 'Texto de busca do campo de filtro embutido.'],
            'empty-text' => ['type' => 'string', 'default' => 'Nenhuma opção disponível', 'notes' => 'Texto exibido quando não há opções ou resultados.'],
            'loading-text' => ['type' => 'string', 'default' => 'Carregando...', 'notes' => 'Mensagem exibida durante o carregamento de opções.'],
            'full' => ['type' => 'bool', 'default' => 'false', 'notes' => 'Expande a largura do componente para 100% do container (w-full).'],
            'type' => ['type' => 'string', 'default' => 'button', 'notes' => 'Tipo HTML do controle ou botão.'],
            'icon-position' => ['type' => 'left|right', 'default' => 'left', 'notes' => 'Posição do ícone em relação ao rótulo textual.'],
            'rounded' => ['type' => 'none|sm|md|lg|full', 'default' => 'md', 'notes' => 'Nível de arredondamento das bordas do elemento.'],
        ];
        ...
        return [
            ...
            'default' => 'null',
            'notes' => 'Propriedade pública configurável do componente.',
        ];
    }
